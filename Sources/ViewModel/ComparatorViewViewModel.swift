...
public final class ComparatorViewViewModel: ObservableObject {
    ...
    private func calculateValues(value: CGFloat, item: ComparatorItemModel) {
        ...
        time = timeTo / 60.0
        guard let charger = item.charger, let estimateCost: EVIOEstimatedCost = EVIOEstimatedCostUtils.shared.calculateEstimatedCost(estimateTime: time, contract: nil, charger: charger, plug: selectedPlug, ev: selectedEv) else {return}
