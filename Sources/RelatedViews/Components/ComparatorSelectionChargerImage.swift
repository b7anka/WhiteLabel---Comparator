...
public struct ComparatorSelectionChargerImage: View {
    ...
    public var body: some View {
        if let urlString: String = self.fav.charger?.defaultImage {
            EVIOImage(urlString: urlString, defaultImage: .chargerDefaultIcon, aspectRatio: .fill, size: CGSize.init(width: 62, height: 62))
                .clipShape(Circle())
                      
        } else {
            EVIOImage(urlString: (self.fav.charger?.imageContent?.first ?? .empty) ?? .empty, defaultImage: .chargerDefaultIcon, aspectRatio: .fill, size: CGSize.init(width: 62, height: 62))
                .clipShape(Circle())
        }
    }
